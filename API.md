...
요청한 사용자가 해당 문제의 편집 권한이 있는 경우, 해당 문제에 파일을 업로드합니다. 만약 기존에 이미 해당 문제에 업로드된 파일이 있다면, 기존 파일을 새로 업로드된 파일로 바꿉니다. **기존 파일은 삭제되어야 합니다.** **파일의 형식은 7z여야 합니다.** 업로드된 파일은 `https://cdn.ctf-archive.com/ctf/:id-:uuid.7z` 에 접속했을 때 다운로드 가능해야 합니다.
...
요청한 사용자가 해당 문제의 편집 권한이 있는 경우, 해당 문제에 빌드 파일을 업로드합니다. 만약 기존에 이미 해당 문제에 업로드된 빌드 파일이 있다면, 기존 파일을 새로 업로드된 파일로 바꿉니다. **기존 파일은 삭제되어야 합니다.** **빌드 파일의 형식은 7z여야 합니다.**
...
### POST /users/:username/profileImage

요청한 사용자가 admin에 속하거나 해당 사용자 본인일 경우, 해당 사용자의 프로필 이미지를 주어진 파일로 변경합니다. **기존 파일은 삭제되어야 합니다.**
...
- { profileImage }
  - profileImage: 변경된 프로필 이미지의 고유번호를 의미합니다.
...
### POST /users/:username/profileBackground

요청한 사용자가 admin에 속하거나 해당 사용자 본인일 경우, 해당 사용자의 프로필 배경 이미지를 주어진 파일로 변경합니다. **기존 파일은 삭제되어야 합니다.**
...
- { profileBackground }
  - profileBackground: 변경된 프로필 배경 이미지의 고유번호를 의미합니다.
