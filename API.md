...
요청한 사용자가 문제 생성 권한이 있는 경우, 주어진 문제를 새로 추가합니다. 이미 해당 문제가 존재한다면, 아무 일도 하지 않습니다. 문제 생성 권한이 존재한다는 것은, 사용자의 auth가 CREATOR 이상이라는 의미입니다.

- title: 문제의 제목입니다.
- source: 문제에 제출할 출처입니다.
- flag: 문제의 답입니다.
- content: 문제의 내용(디스크립션)입니다.
- role: 문제를 편집 및 열람 가능한 role입니다.
- group: 문제의 답입니다.
...
요청한 사용자에게 공개된 문제들 중, 주어진 조건에 맞는 문제의 목록을 반환합니다. 공개되었다는 것은, 요청한 사용자가 admin이거나, 사용자가 문제의 creator거나, 사용자의 groups에 문제의 group이 포함되었다는 뜻입니다.
...
요청한 사용자가 문제 편집 권한이 있는 경우, 해당 문제를 삭제합니다. 문제 편집 권한이 있다는 것은, 요청한 사용자가 admin이거나 사용자가 문제의 creator라는 뜻입니다.
