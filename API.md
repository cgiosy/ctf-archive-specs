...
요청한 사용자가 해당 문제의 편집 권한이 있는 경우, 해당 문제에 파일을 업로드합니다. 만약 기존에 이미 해당 문제에 업로드된 파일이 있다면, 기존 파일을 새로 업로드된 파일로 바꿉니다. 업로드된 파일은 `https://cdn.ctf-archive.com/ctf/dist/:id-:uuid.zip` 에 접속했을 때 다운로드 가능해야 합니다.
