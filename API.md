...
- group: 문제를 편집하거나 공개할 그룹을 변경할 수 있는 그룹입니다.
...
3번에 해당하는 경우(즉, HTTP 메서드가 POST인 경우), 요청의 **Content-Type** 헤더는 반드시 **application/json**이어야 합니다. **아니라면, 상태 코드는 400이며 빈 문자열을 반환하여야 합니다**.
...
요청한 사용자가 문제 생성 권한이 있는 경우, 주어진 문제를 새로 추가합니다. 이미 해당 문제가 존재한다면, 아무 일도 하지 않습니다.
...
요청한 사용자가 해당 문제의 편집 권한이 있는 경우, 문제를 삭제합니다.
...
요청한 사용자가 해당 문제의 열람 권한이 있는 경우, 해당 문제를 반환합니다.
...
### PUT /problems/:problem/submissions
...
- 401: 로그인을 하지 않은 경우입니다. 반환값은 빈 문자열입니다.
- 403: 요청한 사용자에게 해당 문제의 열람 권한이 없는 경우입니다. 반환값은 빈 문자열입니다.
- 403: flag가 잘못된 경우입니다. 반환값은 `WRONG_ANSWER` 입니다.
- 404: 문제가 존재하지 않는 경우입니다. 반환값은 빈 문자열입니다.

### POST /problems/:problem/submissions
...
- 400: problem이 올바르지 않은 id인 경우입니다. 반환값은 빈 문자열입니다.
- 400: levels가 1 이상 30 이하의 자연수가 아닌 경우입니다. 반환값은 빈 문자열입니다.
- 400: comment가 올바르지 않은 경우입니다. 반환값은 빈 문자열입니다.
- 401: 로그인을 하지 않은 경우입니다. 반환값은 빈 문자열입니다.
- 403: 요청한 사용자에게 해당 문제의 열람 권한이 없는 경우입니다. 반환값은 빈 문자열입니다.
- 404: 문제가 존재하지 않는 경우입니다. 반환값은 빈 문자열입니다.

### GET /problems/:problem/submissions

요청한 사용자가 해당 문제의 열람 권한이 있는 경우, 주어진 문제를 푼 사람의 기여를 가져옵니다.
...
요청한 사용자가 admin에 속하거나, user일 경우, 해당 사용자의 정보를 변경합니다.

- { user, achievements, favoriteAchievement, settings, profileImage, profileBackground }
...
요청한 사용자가 admin에 속하거나 user일 경우, 해당 사용자의 이메일을 변경합니다.
...
요청한 사용자가 admin에 속하거나 혹은 user일 경우, 해당 사용자의 비밀번호를 변경합니다.
...
- { type, sessionid, username, settings }
