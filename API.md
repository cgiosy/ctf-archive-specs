...
만약 주어진 입력이 잘못된 쿼리라고 판단된 경우, HTTP 상태 코드는 400 Bad Request 여야 합니다. 또한 __각 API에 명시된 값을 반환__ 하며, 쿼리를 실행하기 전, 파싱 도중에 즉시 응답을 완료하여야 합니다. 즉, 파싱 외의 추가적인 성능 소모가 없어야 합니다.

만약 주어진 입력이 존재하지 않는 요소를 참조한다고 판단된 경우, HTTP 상태 코드는 __404 Not Found__ 여야 합니다. 또한 __각 API에 명시된 값을 반환__ 하며, 쿼리에 대한 결과를 만들기 전, 입력에 따른 요소를 가져오는 도중에 즉시 응답을 완료하여야 합니다. 즉, 파싱 및 값 참조 외에 추가적인 성능 소모가 없어야 합니다.
...
- 첫 번째 단어는 오름차순을 뜻하는 `asc`, 혹은 내림차순을 뜻하는 `desc` 입니다.
  - 400: `asc` 혹은 `desc` 가 아닌 경우.
- 두 번째 단어는 아래 목록 중에 있어야 합니다.
  - id:
    - 설명이 필요하지 않을 듯 하므로 생략합니다.
  - level:
    - 문제를 정렬할 때는 난이도를 의미합니다.
    - 사용자를 정렬할 때는 레벨을 의미합니다.
    - 대회를 정렬할 때는 문제의 평균 난이도를 의미합니다.
  - solves:
    - 문제를 정렬할 때는 푼 사람의 수를 의미합니다.
    - 사용자를 정렬할 때는 푼 문제의 수를 의미합니다.
    - 400: 대회를 정렬하는 경우.
  - 400: 위의 어느 하나에도 해당하지 않는 경우.
...
각 이메일은 최대 하나의 코드를 가질 수 있습니다. 자세한 설명은 API의 `/authenticate_email` 를 참고하세요.
...
1. 주소에서 /로 구분된 단어들 중 :로 시작하는 것
  ...
  - 예시: `/dragon/:dragon_id/likes/:category` 에서 `dragon_id` 와 `category` 는 1번에 해당하는 입력입니다.

2. HTTP 메서드가 GET: 주소에서 ? 뒤에 오는 것들, 즉 query string

  - 예시: `/snake/:snake_id/:snake_type?start=5&limit=60` 에서 `start` 와 `limit` 는 2번에 해당하는 입력입니다.

3. HTTP 메서드가 POST: HTTP Body. JSON 형태

  - 예시: `/write/:post_id` 주소로 body가 `{"title":"asdf","content":"asdf"}` 인 요청이 들어왔을 때 `title` 과 `content` 는 3번에 해당하는 입력입니다.

1번은 제목(주소)에 명시적으로 적혀 있습니다. 2번과 3번은 제목에 적기엔 너무 많거나 지저분해져 생략합니다. 즉, 1번에 해당하지 않는 입력은 2번 혹은 3번으로 들어온다고 생각해 주세요. 또한 HTTP 메서드가 GET이면서 POST일 수는 없기 때문에, 각 요청에서 1번 형태가 아닌 입력은 반드시 2번이나 3번 중 하나의 형태로만 들어옵니다.

3번에 해당하는 경우(즉, HTTP 메서드가 POST인 경우), **Content-Type** 헤더는 반드시 **application/json**이어야 합니다. **아니라면, 상태 코드는 400이며 빈 문자열을 반환하여야 합니다**.

### GET /problems/:query

주어진 조건에 맞는 문제의 목록을 반환합니다.
...
- count: query를 만족하는 문제의 개수입니다. sort 혹은 page에 영향을 받지 __않습니다__.
- problems: query를 만족하는 문제 정보들을 sort에 따라 정렬했을 때, [30 * (page - 1), 30 * page) 구간 배열입니다. 문제 정보는 다음과 같은 객체입니다.
...
### GET /problem_content/:problem

해당 문제의 설명(디스크립션)을 반환합니다.
...
### GET /check_flag/:problem/:flag

주어진 문제에 답을 제출합니다. 답이 맞는지 판별하는 것 외에 아무 일도 하지 않습니다.
...
주어진 문제에 답을 제출합니다. 답이 틀린 경우 별도로 아무 일도 하지 않으며, 맞다면 DB에 제출을 추가합니다. 제출은 다음 의사코드와 동일한 일을 수행하는 [TRIGGER](https://www.postgresql.org/docs/current/sql-createtrigger.html)를 작동시켜야 합니다.
...
사용자의 정보를 받아 가입시킵니다. 성공적으로 가입한 후에는, `/login` 을 실행시킨 것과 동일한 작업을 수행해야 합니다.
...
성공 시 `/login` 과 동일한 형식의 결과를 반환합니다.

실패 시 처리는 다음과 같습니다.

- 400: 해당 이메일이 존재하지 않거나, 해당 이메일로 `/authenticate_email` 을 통해 전송한 코드와 주어진 코드가 일치하지 않는 경우입니다. 반환값은 빈 문자열입니다.
